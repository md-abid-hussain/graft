...
**ten typed tools and two resources**, reads and writes together. Six read tools pass
straight through; four write tools carry no `readOnlyHint`, so the harness gates every
one behind human approval. Every tool declares an `outputSchema` and returns typed
records, a bad slug is refused with the valid options, and ingestion takes URLs —
never page content.

The full reference — every tool's purpose, both resources, and the four design
decisions (schema-as-contract, `llms-full.txt` only, hybrid retrieval, and
discovery-vs-fetching) — lives in **[contract.md](contract.md)**.
